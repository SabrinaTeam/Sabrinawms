   ...
   public class sizeRunService
	{
		public DataTable getSizeRunByMy_no(string my_no,string linkServer)
		{
		  ...
		  //  DataTable dt = BEST_SqlHelper.ExcuteTable(sqlstr);
			...
			if (linkServer == "BESTconnStr_KM")
			{
				dt = BEST_SqlHelper_KM.ExcuteTable(sqlstr);
			}
			else if (linkServer == "BESTconnStr")
			{
				dt = BEST_SqlHelper.ExcuteTable(sqlstr);
			}
		...
		public DataTable getClr_noByMy_no(string[] parameters, string linkServer)
		{
		   ...
		   if (linkServer == "BESTconnStr_KM")
			{
				 dt = BEST_SqlHelper_KM.ExcuteTable(sqlstr, paras);
			}
			else if(linkServer == "BESTconnStr")
			{
				 dt = BEST_SqlHelper.ExcuteTable(sqlstr, paras);
			}
