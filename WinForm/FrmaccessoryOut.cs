...
namespace WinForm
{
    public partial class FrmaccessoryOut : Form
    {
        ...
        private List<allqtys> allqtys = new List<allqtys>();
        ...
        public void DoWork_getAccessOut(object sender, DoWorkEventArgs args)
        {
            ...
            else
            {
               ...
               new Action(
                    ...
                    // 1、台北服务器 2、柬埔寨服务器 慢 备用 List<materials> groupMaterials = 
                    // 柬埔寨服务器 慢 备用 查询自编单号下的所有物料
                    ...
                    if (groupMaterials != null && groupMaterials.Count > 0)
                    {
                        ...
                        for (int i = 0; i < groupColorsNames.Count; i++)
                        {
                            ...
                            else
                            {
                                for (int j = 0; j < groupColorsMaterials.Count; j++)
                                {
                                    ...
                                    // 如果这个 色组的物料ID与颜色 跟所有物料集合里的ID 颜色 对得上 添加上去 如果
                                    // 所有物料集合里的某个物料跟所有色组里的物料都对不上，添加上去， 注明 改
                        ...
                        // 如果色组不同 但是物料的颜色相同 数量是加在一起的 添加物料的采购单 供应商 等信息 分群 单位用量
                                ...
                                for (int j = 0; j < purs.Count; j++)
                                {
                                        ...
                                        groupColorsMaterialAll[i].od_qty == purs[j].od_qty)
                                    {
                                        ...
                                        groupColorsMaterialAll[i].od_qty = 0; //总订单数量
                                                                              // groupColorsMaterialAll[i].od_Finished_qty
                                                                              // = 0; //已生产数量
                                                                              // 本地库拉出累计生产数量
                                                                              // groupColorsMaterialAll[i].od_Unfinished_qty
                                                                              // = 0; //剩余数量 总订单数量 -
                                                                              // 已生产数
                                                                              // groupColorsMaterialAll[i].od_make_qty
                                                                              // =
                                                                              // Convert.ToInt32(this.txtManufactures.Text.Trim());
                                                                              // //本批生产数量
                                                                              // groupColorsMaterialAll[i].od_make_qty
                                                                              // = 0; //本批生产数量

                                        // groupColorsMaterialAll[i].od_Exceed_qty =
                                        // groupColorsMaterialAll[i].od_make_qty -
                                        // (groupColorsMaterialAll[i].od_Unfinished_qty +
                                        // groupColorsMaterialAll[i].od_make_qty); //溢出数量
        ...
        private void runMake(List<materials> groupColorsMaterialAll, string od_no, int? qty, int? makeQty)
        {
            ...
            if (localhostMaterialAll == null || localhostMaterialAll.Count <= 0)
            {
                for (int i = 0; i < groupColorsMaterialAll.Count; i++)
                {
                    ...
                    // 计算应发料量 groupColorsMaterialAll[i].mas_qty = groupColorsMaterialAll[i].unit_qty
                    // * groupColorsMaterialAll[i].style_mas_qty * (groupColorsMaterialAll[i].loss /
                    // 100 + 1) / groupColorsMaterialAll[i].trans_rate; //本次应发料量
                    int? nowmake = makeQty;
                    ...
                    // masUnfinishedQty = groupColorsMaterialAll[i].pu_trans_qty -
                    // (groupColorsMaterialAll[i].masFinished_qty +
                    // groupColorsMaterialAll[i].mas_qty); //剩余料量
                ...
                // 这里进行过滤重复的值 List<materials> lister = groupColorsMaterialAll;
        ...
        public void upDataFromERP()
        {
            ...
            if (accessorytb != null && accessorytb.Count > 0)
            {
                // 2、有资料 已有数据 是否需要更新 软删除
                if (MessageBox.Show("已有数据  是否需要更新?", "提示", MessageBoxButtons.YesNo) ==
                    DialogResult.Yes)
                {
                    // 标记状态为E 表示有错误，已不再使用了 。软删除 物料发放状态 O完成 N未发料 P发了一部分 进行中 E表示有错误 有修改 不再使用的
        ...
        public bool testOpenDB(string serName)
        {
            ...
            {
                // for (int i = 0; i < list.Count; i++) { MessageBox.Show("服务器连接成功,数据库打开成功"); }
                return true;
            }
        }
        ...
        public void changHeaderText()
        {
            ...
            // this.dgvAccessory.Columns["od_make_qty"].DefaultCellStyle.ForeColor =
            // System.Drawing.Color.Blue; this.dgvAccessory.Columns["od_Exceed_qty"].HeaderText =
            // "溢出数量"; this.dgvAccessory.Columns["od_Exceed_qty"].DefaultCellStyle.ForeColor = System.Drawing.Color.Red;
        ...
        private void txtManufactures_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                else
                {
                    ...
                    // for ( int i=0;i< dt.Rows.Count; i++) { material.id = dt.Rows[i][] }
        ...
        {
            // MessageBox.Show(this.cellstr); this.dgvAccessory.CurrentCell.Value.ToString();
            double s = 0.00;
            double c = 0.00;
        ...
        private void btConfirmOut_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < tb.Rows.Count; i++)
            {
                if (tb.Rows[i]["select"].ToString() == "True")
                {
                    ...
                    //tb.Rows[i]["mas_qty"] = 0;
                     item.a_id = tb.Rows[i]["id"].ToString() =="" ?  0 : Convert.ToInt32(tb.Rows[i]["id"].ToString());
                    ...
                    mitem.id = tb.Rows[i]["id"].ToString() == "" ? 0 : Convert.ToInt32(tb.Rows[i]["id"].ToString());
        ...
        {
            // this.btPrint.Enabled = false; this.btPrint.Visible = false; this.btConfirmOut.Enabled
            // = false;
        }
